...
class ChannelSenderTest {
    ...
    @Test
    fun `duplicate sends to all channels in parallel`() = runTest {
        val sentTo = mutableListOf<String>()
        val r = ChannelSender.send(
            "hi", token, chatId,
            listOf(channel("d", "Direct"), channel("p1", "Proxy 1"), channel("p2", "Proxy 2")),
            duplicate = true,
        ) {
            synchronized(sentTo) { sentTo.add(it.name) }
            ChannelSender.ChannelOutcome.Sent(1)
        }
        assertTrue(r is ChannelSender.Result.Ok)
        assertEquals(3, sentTo.size)
        assertTrue(sentTo.containsAll(listOf("Direct", "Proxy 1", "Proxy 2")))
    }

    @Test
    fun `duplicate success if at least one channel succeeds`() = runTest {
        val r = ChannelSender.send(
            "hi", token, chatId,
            listOf(channel("d", "Direct"), channel("p1", "Proxy 1")),
            duplicate = true,
        ) {
            if (it.id == "d") ChannelSender.ChannelOutcome.Failed("nope")
            else ChannelSender.ChannelOutcome.Sent(42)
        }
        assertTrue(r is ChannelSender.Result.Ok)
        assertEquals("Proxy 1", (r as ChannelSender.Result.Ok).channelName)
    }

    @Test
    fun `duplicate fails only if all channels fail`() = runTest {
        val r = ChannelSender.send(
            "hi", token, chatId,
            listOf(channel("d", "Direct"), channel("p1", "Proxy 1")),
            duplicate = true,
        ) {
            ChannelSender.ChannelOutcome.Failed("error")
        }
        assertTrue(r is ChannelSender.Result.Err)
        assertEquals(2, (r as ChannelSender.Result.Err).reasons.size)
    }

    @Test
    fun `duplicate with empty channels returns error`() = runTest {
        val r = ChannelSender.send(
            "hi", token, chatId,
            emptyList(),
            duplicate = true,
