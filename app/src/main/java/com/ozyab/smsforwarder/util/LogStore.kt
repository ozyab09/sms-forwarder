 ...
 * и в кольцевой буфер для UI: последние [MAX_ENTRIES] записей. Буфер
 * in-memory (privacy-first: логи никогда не пишутся на диск), поэтому
 * хранится столько, сколько помещается — при активной работе это часы,
 * а не дни. [RETENTION_MS] страхует старые записи, если буфер не успел
 * заполниться. Ручная очистка — clear() (кнопка во вкладке «Логи»).
 */
object LogStore {
