...
- 📩 **Входящие SMS** — мгновенно, с именем отправителя из контактов
- 📤 **Исходящие SMS** — пересылает отправленные сообщения
- 📵 **Пропущенные вызовы** — только реально не отвеченные
- 📞 **Принятые входящие звонки** — когда вы ответили на вызов
- 📞 **Исходящие звонки** — когда вы сами звоните
...
- 🔄 **Каскадные ретраи** — пробует каналы по порядку, экспоненциальный бэкофф 15с→30с→60с (кап 10 мин), суммарно ~15 мин; порядок каналов настраивается сам: успешный поднимается наверх, неудачный уходит вниз
...
- 📊 **Вкладка «Логи»** — INFO/OK/WARN/ERROR, in-memory буфер последних 200 записей
...
- 🎨 **Акцентные цвета** — бирюзовый, зелёный, красный, синий, фиолетовый, оранжевый, серый
- 🧩 **Шаблоны сообщений** — `{sender}`, `{text}`, `{time}`, `{name}`, `{sim}`, `{number}` и другие; кнопки «Проверить» (превью сообщения), «Сохранить» и «Сбросить»
...
| **Переключатели** | Вкл/выкл входящие SMS, исходящие SMS, пропущенные вызовы, принятые входящие, исходящие звонки. |
| **Шаблоны сообщений** | 5 кастомных форматов: входящие/исходящие SMS, пропущенные/принятые/исходящие вызовы. Плейсхолдеры: `{sender}`, `{name}`, `{text}`, `{time}`, `{date}`, `{type}`, `{sim}`, `{number}`. |
...
| **Темы** | Светлая / тёмная / по системе + акцентные цвета (во вкладке «О приложении»). |
...
|---------|----------------|
| Pull Request | Debug APK + unit-тесты + lint (обязательная зелёная проверка) |
| Merge в main | Создание тега по версии из `gradle/libs.versions.toml` |
| Тег `vX.Y.Z` | Unit-тесты → подписанный Release APK → GitHub Release |

Версию bump'ит автор в том же PR, где фича/фикс. `versionCode` = `major*10000 + minor*100 + patch`.
...
| **Превью шаблонов** | Кнопка «Проверить» показывает, как будет выглядеть SMS/пропущенный вызов | Низкая | ✅ v0.4.30 |
| **Детали события в истории** | Полный текст, Chat ID, бот, канал — по клику на событие | Низкая | ✅ v0.4.32 |
| **Мульти-бот** | Несколько ботов с разными Chat ID (разные получатели) | Средняя | ❌ не планируется |
...
| **Дублирование каналов** | Параллельная отправка в 2+ ботов одновременно | Средняя | ❌ удалено (v0.5.22 — не работало: нет разных Chat ID для каналов) |
| **Push-уведомления на телефон** | Локальные уведомления при получении SMS/вызова | Низкая | ✅ v0.5.0 |
| **Поиск по логам** | Фильтр по уровню/тексту/времени во вкладке «Логи» | Низкая | ✅ v0.5.0 |
