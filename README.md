...
|---------|----------------|
| Pull Request | Debug APK + тесты (артефакт) |
| Push в main | Auto-bump patch → тег → Release APK + GitHub Release |

Версия берётся из `gradle/libs.versions.toml`. `versionCode` = `major*10000 + minor*100 + patch`.
...
| **Дублирование каналов** | Параллельная отправка в 2+ ботов одновременно | Средняя | ✅ v0.5.0 |
