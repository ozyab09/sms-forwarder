...
- 🚫 **Ничего лишнего** — никаких локальных оповещений и аналитики, только пересылка
...
- ⏹️ **Полная остановка** — одна кнопка «Запустить/Остановить»: сразу видно, работает ли пересылка; «Стоп» останавливает её до следующего «Запустить» (новые SMS/звонки не возобновят)
- ✂️ **Длинные сообщения** — автоматическая нарезка на части по лимиту Telegram 4096 символов
- 🧩 **Панели интерфейса** — «Каналы» и «История» выделены в отдельные компоненты, MainActivity — тонкая склейка
...
| **Статус** | Одна кнопка «Запустить»/«Остановить» — видно, работает пересылка или нет. «Стоп» останавливает до следующего «Запустить». |
...
│   └── util/        # Prefs, LogStore, ThemeManager, QuietHours, SettingsBackup, TemplateFormatter, SimInfo, ContactNames
...
| **Push-уведомления на телефон** | Локальные уведомления при получении SMS/вызова | Низкая | ❌ удалено (v0.6.7 — не используется) |
